...
def get_current_session(user: User):
    ...
    # If is not blank, it is a current session (vs a new one) and the bot continues the chat with the current user
    if is_not_blank(session_id):
        return Session(session_id, False)
    else:
        # Creating a random string as the new session id for the new chat
